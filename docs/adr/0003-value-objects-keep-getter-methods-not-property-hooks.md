...
Collections differ in *shape*, not in principle: a typed collection exposes the iterable and countable surface (`IteratorAggregate`/`Countable`) plus `toArray()`/`contains()`, rather than a `getItems()` accessor.
